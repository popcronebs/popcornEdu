...
->orderBy('id')
->get();
$sel_student = (session()->get('student_seq') ? $students->where('id', session()->get('student_seq'))->first() : $students[0]);

if (session()->get('student_seq') == null ){
    if($sel_student->student_seq != null) session()->put('student_seq', $sel_student->student_seq);
    else session()->put('student_seq', $students[0]->id);
}
@endphp
...
<style>
    ...
    /* rotate 시 애니메이션  */
    [data-parent-layout-profile] button {
        ...
        <nav id="layout_main_top" class="navbar navbar-expand-sm bg-primary-y navbar-light d-none">
            <div class="container-fluid main_top_p">
                ...
                <div class="col collapse navbar-collapse justify-content-end" id="collapsibleNavbar">
                    <ul class="navbar-nav">
                        {{-- 상단 / 메뉴 가져오기. --}}
                        @foreach ($menus->where('menu_type', 'top')->where('is_use', 'Y') as $idx => $menu)
                    ...
                    <a class="btn btn-primary" href="/parent/logout">로그아웃</a>
                </div>
            </div>
        </nav>
        <nav id="layout_main_top2" class="row border align-items-center main_top_p mx-0">
            {{-- 메뉴 쪽 로고 --}}
            <a class="col-auto" href="/parent">
                <img src="{{ asset('/images/popcorn_logo.svg') }}" alt="">
            </a>
            {{-- <span class="fs-6 col-auto p-0" style="color:#ffb357">For parent</span> --}}

            {{-- 디자인으로 인해 삭제 --}}
            {{-- <span class="col-auto ps-5">{{ session()->get('student_name') }}</span> /
            ...
            <div class="col d-none d-md-flex justify-content-end gap-5" id="main-menu">
                <div class="nav-wrap d-flex justify-content-end">
                    @if(!empty($menus))
                        @foreach ($menus->where('menu_type', 'nav')->where('is_use', 'Y')->where('menu_pt_seq', 0) as $menu)
                        ...
                        @if($menus->where('menu_pt_seq', $menu['id'])->where('is_use', 'Y')->count() > 0)
                            ...
                            {{ $is_menu_gray ?
                                ($now_menu_pt_seq == $menu['id'] ? 'primary-text-mian' : 'scale-text-black'):'' }}" menu_seq="{{ $child_first['id'] }}" pt_seq="{{ $child_first['menu_pt_seq'] }}" id="menu-{{ $child_first['id'] }}" data-menu-url="{{ $child_first['menu_url'] }}" onclick="openWindow('{{ $child_first['menu_url'] }}', '{{ $child_first['menu_url_code'] }}', '{{ $child_first['is_blank'] }}','Y')">
                            {{ $menu['menu_name'] }}
                        @else
                            {{-- 폴더아 아닌 메뉴 --}}
                            {{ $is_menu_gray ?
        ...
        <div class="p-4">
            <div class="user-info row">
                <div class="px-0 col-auto">
                    <div class="btn btn_top_icon p-0 ms-2 border-primary-y rounded-circle border-2 h-center">
                        @if($parent->profile_img_path)
                        <img src="/storage/uploads/user_profile/parent/{{ $parent->profile_img_path}}" class="rounded-circle" width="52px">
                        @else
                        <img src="{{ asset('images/yellow_human_icon.svg')}}" class="rounded-circle" width="52px">
                        @endif
                    </div>
                </div>
                <div class="user-details col">
                    <div class="text-sb-18px text-black mt-1" data-layout-profile="student_name">{{ $sel_student->student_name }}학생</div>
                    <div class="text-sb-16px scale-text-gray_05 mt-2" data-layout-profile="grade_name"> {{$sel_student->grade_name}}</div>
                </div>
                <div class="mt-4 pt-2 px-0" style="max-height:300px;overflow-y:auto;">
                    @if(!empty($students))
                    @foreach($students as $student)
                    <div class="h-center gap-3 text-sb-16px p-2 scale-bg-gray_01 scale-text-gray_05 rounded-2">
                        <label class="radio">
                            <input onchange="parentLayoutChgChild(this);"
                                data-parent-layout-student-seq="{{$student->id}}" data-parent-layout-student-name="{{$student->student_name}}" data-parent-layout-grade-name="{{$student->grade_name}}"
                                type="radio" value="only_exam_print" name="wrong_print_radio"
                                {{ count($students) == 1 || (session()->get('student_seq') == $student->id) ? 'checked':''}}>
                            <span class=""></span>
                        </label>
                        <span> {{$student->student_name}}({{$student->grade_name}})</span>
                    </div>
                    @endforeach
                    @endif
                </div>
                {{--  2명 이상일때, 아무도 선택이 안되어 있으면, 첫번째 학생 선택. --}}
                <script>
                if (document.querySelectorAll('[data-parent-layout-student-seq]').length > 0
                && document.querySelectorAll('[data-parent-layout-student-seq]:checked').length == 0) {
                        document.querySelector('[data-parent-layout-student-seq]').checked = true;
                }
                </script>
            </div>
            <ul class="menu-items mt-4" data-parent-layout-profile>
                <li class="menu-item">
                    <div class="h-center gap-2">
                        <div class="col-auto">
                            <img src="{{asset('images/yellow_human_icon.svg')}}" style="filter: grayscale(1);" width="32">
                        </div>
                        <div class="col text-sb-18px"> 회원 정보 관리 </div>
                        <div class="col-auto">
                    ...
                    <ul class="submenu text-sb-16px scale-text-gray_05" hidden>
                        <li class="mt-2 ps-4 ms-3 cursor-pointer" data-layout-menu="/parent/member/info" onclick="parentLayoutPageMove(this, 'parent') ">학부모 정보 관리</li>
                        <li class="mt-2 ps-4 ms-3 cursor-pointer" data-layout-menu="/parent/member/info" onclick="parentLayoutPageMove(this, 'child')">자녀 정보 관리</li>
                    </ul>
                </li>
                ...
                <li class="menu-item mt-3">
                    <div class="h-center gap-2">
                        ...
                        <div class="col text-sb-18px cursor-pointer" data-layout-menu="/parent/noti/settings" onclick="parentLayoutPageMove(this);">알림 설정</div>
                ...
                <li class="menu-item mt-3">
                    <div class="h-center gap-2">
                        ...
                        <div class="col text-sb-18px cursor-pointer" data-layout-menu="/teacher/messenger" onclick="parentLayoutPageMove(this)">응원 메시지 보내기</div>
                        <div class="col-auto" hidden>
                            <img src="{{ asset('images/dropdown_arrow_down.svg') }}" style="filter: grayscale(1);" width="24px">
                        </div>
                    </div>
                ...
                <li class="menu-item mt-3">
                    <div class="h-center gap-2">
                        ...
                        <div class="col text-sb-18px cursor-pointer" data-layout-menu="/teacher/messenger" onclick="parentLayoutPageMove(this)">1:1 학습 상담</div>
                        <div class="col-auto" hidden>
                            <img src="{{ asset('images/dropdown_arrow_down.svg') }}" style="filter: grayscale(1);" width="24px">
                        </div>
                    </div>
                ...
                <li class="menu-item mt-3">
                    <div class="h-center gap-2">
                        ...
                        <div class="col text-sb-18px cursor-pointer" data-layout-menu="/parent/push/list" onclick="parentLayoutPageMove(this)">알림센터</div>
                        <div class="col-auto" hidden>
                            <img src="{{ asset('images/dropdown_arrow_down.svg') }}" style="filter: grayscale(1);" width="24px">
                        </div>
                    </div>
                ...
                <li class="menu-item mt-3">
                    <div class="h-center gap-2">
                        <div class="col-auto">
                            <img src="{{asset('images/yellow_point_icon.svg')}}" style="filter: grayscale(1);" width="32">
                        </div>
                        <div class="col text-sb-18px cursor-pointer" onclick="parentLayoutPageMove(this)">포인트</div>
                        <div class="col-auto" hidden>
                            <img src="{{ asset('images/dropdown_arrow_down.svg') }}" style="filter: grayscale(1);" width="24px">
                        </div>
                    </div>
...
<script>
    ...
    // if(document.querySelector('[data-parent-layout-student-seq]:checked'))
    //     parentLayoutChgChild(document.querySelector('[data-parent-layout-student-seq]:checked'), true);

    function openWindow(page_url, page_code, is_blank,is_folder) {
        if (is_blank == null) is_blank = 'N';
        if(is_folder){
            page_url += '?is_folder=Y';
        }
        if (is_blank == 'N') {
    ...
    // 학부모 프로필 메뉴 > 자녀 선택
    function parentLayoutChgChild(vthis, is_not_toast) {
        const student_seq = vthis.dataset.parentLayoutStudentSeq;
        const student_name = vthis.dataset.parentLayoutStudentName;
        const grade_name = vthis.dataset.parentLayoutGradeName;
        const page = "/parent/change/child";
        const parameter = {
            student_seq: student_seq,
        };
        queryFetch(page, parameter, function(result) {
            if ((result.resultCode || '') == 'success') {
                if (!is_not_toast) toast('선택 자녀가 변경되었습니다.');
                //비동기로 처리하려 했으나, 어차피, 각 페이지를 모두 변경해줄수 없으므로, 새로고침.
                // document.querySelector('[data-top-profile="student_name"]').innerHTML = student_name;
                // document.querySelector('[data-top-profile="grade_name"]').innerHTML = grade_name;
                // document.querySelector('[data-layout-profile="student_name"]').innerHTML = student_name + ' 학생';
                // document.querySelector('[data-layout-profile="grade_name"]').innerHTML = grade_name;
    ...
    // 학부모 프로멜 메뉴 하위 메뉴 오픈
    function parentLayoutProfileMenuOpen(vthis) {
        const pt_li = vthis.closest('li');
        const ul = pt_li.querySelector('ul');
    ...
    // 페이지 이동
    function parentLayoutPageMove(vthis, type) {
        const page_url = vthis.dataset.layoutMenu;
        location.href = page_url+(type != undefined ? `?type=${type}`:'');
    }
</script>
