...
<style>
    ...
    @media (max-width: 1400px) {
        .score-grid-item span span {
            transform: scale(0.8);
        }
    }
</style>

<input type="hidden" data-main-student-lecture-detail-seq value="">
<input type="hidden" data-main-exam-lecture-detail-seq value="">
<input type="hidden" data-main-student-exam-seq="">

<div class="col mx-0 row position-relative">

    <div class="col-lg col-lg-9 pe-0 ">
        {{-- 동영상 시청 --}}
        <section data-section-study-video="top" class="">
            ...
            <div class="w-100 overflow-hidden video-container" style="" id="videoPtDiv">
                <ul class="video-cont-tab cursor-pointer">
                    <li onclick="" class="assessment active" data-type="unit_test"><span>1</span> 단원평가</li>
                </ul>
            </div>
        </section>
        <section class="video-container">
            <div class="video-title d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    <span class="subject-name">{{$exam->code_name}}</span>
                    <span class="lecture-name text-b-24px">{{$title}}</span>
                </div>
...
const quizData = [
    ...
    @foreach($normals as $normal)
    {
        ...
        choices: [
            ...
            "{{$choice}}",
            @endforeach
            @if(optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample1')->first())->file_path)
            "<img src='{{ optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample1')->first())->file_path }}'>",
            @endif
            @if(optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample2')->first())->file_path)
            "<img src='{{ optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample2')->first())->file_path }}'>",
            @endif
            @if(optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample3')->first())->file_path)
            "<img src='{{ optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample3')->first())->file_path }}'>",
            @endif
            @if(optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample4')->first())->file_path)
            "<img src='{{ optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample4')->first())->file_path }}'>",
            @endif
            @if(optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample5')->first())->file_path)
            "<img src='{{ optional($exam_uploadfiles->where('exam_detail_seq', $normal->id)->where('file_type', 'sample5')->first())->file_path }}'>",
            @endif
        ],
        answer: [
...
const challengeQuizData = [
    ...
    @foreach($challenges as $challenge)
    {
        ...
        question: @json($challenge->questions),
...
function makeExam(index = 0, type = 0, data = quizData){
    ...
    if(currentQuestion != undefined){
        ...
        tryText.innerHTML = `${currentQuestion?.questionNumber}. ${currentQuestion?.question}`;
...
function arrowUpdateNew(index, currentQuestion, answerData) {
    ...
    if(!is_grading || semiQuizData[index] == undefined){
        prev.dataset.arrowPrev = index - 1;
        next.dataset.arrowNext = index;
        next.dataset.arrowNext++
        next.disabled = next.dataset.arrowNext == quizData.length;
    ...
    scoreGrid.innerHTML = quizData.map((question, idx) =>{
        ...
        rtn_str += `
            ...
            if(semiQuizData[idx]?.student_answer?.join(';') != undefined){
                 rtn_str += `유사</span>
                            <div class="col all-center px-2 border-start border-bottom">
                                ${
                ...
                // TODO: max_width 76px 임시방편. UI 추후 수정.
                 rtn_str += `도전</span>
                            <div class="col all-center px-2" style="max-width:65px">
                                ${
                ...
                `;
                 ...
                 rtn_str += `-</span>
                            <div class="col all-center border-start border-bottom px-2">
                                <span class="col pe-2 all-center marking2"><span class="d-inline-block"></span></span>
...
function lastExamResult(){
    ...
    if(is_grading && last_num == exam_num){
        ...
        // if(is_grading && semiQuizData[idx] == undefined){
        //     is_end = true;
        //     isEnd = true;
        // }
    }
...
function studyVideoExit() {
    ...
    sAlert('', msg1 + msg2, 3, function() {
            ...
            document.querySelector('.modal-backdrop').remove();
                ...
                location.href = '/student/unit/test';
        },
        '더 해볼게요.',
        '네,그만할게요.');
    // 배경음영.
